...
PROMPT_TEMPLATES["knowledge_base_chat"] = {
    "default":
        """
        <指令>你是一个电力公司相关的专家，请完全依据已知信息的内容，先找出与问题相关的信息，然后再根据这些相关信息回答简洁、专业地来回答问题。如果无法从中得到答案，请说 “根据已知信息无法回答该问题”，不允许在答案中添加编造成分，不回答与问题无关的内容,请使用中文
简体回答. </指令>
        <已知信息>{{ context }}</已知信息>、
        <问题>{{ question }}</问题>
        """,
...
PROMPT_TEMPLATES["data_augment"] = {
    "default":
        """
        <指令>参照原始句子，简洁生成一个与原始句子语义最相似而句子结构又不同于原始句子的一个新句子，答案请使用简体中文，直接给出答案而不用复述原始句子内容，并且新句子有助于提高检索的准确度</指令>
        <原始句子>{{question}}</原始句子>
        """,
}
